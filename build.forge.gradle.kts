...
base {
    archivesName.set("mutualzz-voice-forge")
}
...
sourceSets {
    main {
        java {
            ...
            exclude { element ->
                val f = element.file
                val inCommon = f.invariantSeparatorsPath.contains("/common/src/main/java/")
                inCommon && (
                    f.name == "VoicePayload.java"
                        || f.name == "VoiceClientController.java"
                        || f.name == "VoiceSettingsScreen.java"
                    )
            }
        }
        resources {
