...
/**
 * NeoForge client entry — shares audio/session/HUD with Fabric via {@code common}.
 * NeoForge {@link KeyModifier} is single-key only (no Alt+Shift combos like Amecs on Fabric),
 * so deafen / mic-sensitivity defaults use Shift instead of Alt+Shift.
 */
...
public final class MutualzzVoiceNeoForge {
    ...
    public MutualzzVoiceNeoForge(IEventBus modBus) {
        ...
        VoiceNetworking.setBackend(json -> ClientPacketDistributor.sendToServer(new VoicePayload(json)));
    }

    private void registerPayloads(RegisterPayloadHandlersEvent event) {
        // Paper owns the real server handler; stub is for NeoForge registration only.
        event.registrar("1")
                .playBidirectional(
                        VoicePayload.TYPE,
                        VoicePayload.CODEC,
    ...
    private void registerClientPayloadHandlers(RegisterClientPayloadHandlersEvent event) {
        event.register(
                VoicePayload.TYPE,
                HandlerThread.MAIN,
                (payload, context) -> VoiceSession.handlePluginMessage(payload.json())
        );
    }
