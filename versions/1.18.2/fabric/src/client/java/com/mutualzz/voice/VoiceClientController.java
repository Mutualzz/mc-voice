...
public final class VoiceClientController {
    ...
    public static void renderHud(HudCanvas graphics) {
        ...
        int y = 16;
        graphics.drawText(
                "Vol " + pct(VoiceSettings.get().outputVolume()) + "% · Mic "
                        + pct(VoiceSettings.get().micSensitivity()) + "%",
                4, y, 0xFFAAAAAA, true
        );
        y += 10;
    ...
    private static void chat(Minecraft client, String message) {
        if (client.player != null) {
            client.player.sendMessage(new net.minecraft.network.chat.TextComponent("[Mutualzz] " + message), net.minecraft.Util.NIL_UUID);
        }
    }
}
