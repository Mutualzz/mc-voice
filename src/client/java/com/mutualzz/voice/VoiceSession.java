...
public final class VoiceSession {
    ...
    private static void syncVoiceStateToServer() {
        ...
        try {
            VoicePayload.sendVoiceState(selfMute, selfDeaf);
    ...
    public static void handlePluginMessage(String json) {
        try {
            ...
            tellPlayer("Voice message error: " + e.getMessage());
        }
    }
    ...
    private static synchronized void join(String audioWsUrl, String audioToken, boolean isReconnect) {
        ...
        try {
            ...
            tellPlayer(isReconnect
                    ...
                    : "Connected to Mutualzz voice audio");
        } catch (Exception e) {
            connected = false;
            Throwable root = e;
            ...
            if (isPermanentDisconnect(-1, detail) || isPermanentDisconnect(-1, root.getMessage())) {
                ...
                cancelReconnectThread();
                leave(true);
                tellPlayer("Voice connect rejected (" + where + "): " + root);
                return;
            }
            if (sessionWanted) {
                state = ConnectionState.RECONNECTING;
                tellPlayer("Voice connect failed (" + where + "): " + root + " — retrying…");
                scheduleReconnect();
            } else {
                leave(true);
                tellPlayer("Failed to connect voice audio (" + where + "): " + root);
            }
        }
    }
    ...
    private static void onSocketLost(String reason) {
        ...
        state = ConnectionState.RECONNECTING;
        tellPlayer("Voice audio lost" + (reason == null || reason.isBlank() ? "" : ": " + reason)
                + " — reconnecting…");
        scheduleReconnect();
    }
        ...
        uplinkThread = new Thread(() -> {
            while (connected && !Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    tellPlayer("Mic uplink send failed: " + e.getMessage());
                    try {
                        Thread.sleep(200);
    ...
    private static final class Listener implements WebSocket.Listener {
        ...
        public void onError(WebSocket webSocket, Throwable error) {
            ...
            if (!sessionWanted) {
                state = ConnectionState.IDLE;
                tellPlayer("Voice audio error: " + error.getMessage());
                return;
            }
